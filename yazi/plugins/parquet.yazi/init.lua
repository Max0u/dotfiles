...
function M:peek()
	...
	if not pqrs_child then
		return self:fallback_to_builtin()
	end

	local jq_child = Command("jq")
		:args({
			"-C"
		})
		:stdin(pqrs_child:take_stdout())
		:stdout(Command.PIPED)
		:stderr(Command.PIPED)
		:spawn()

	if not jq_child then
		return self:fallback_to_builtin()
	end
	...
	local i, lines = 0, ""
	repeat
		local next, event = jq_child:read_line()
		if event == 1 then
			return self:fallback_to_builtin()
		elseif event ~= 0 then
			break
